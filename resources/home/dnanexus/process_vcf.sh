...
then
    ...
    if [ "$2" != "NA" ] && [ "$3" != "NA" ]
    then
        bcftools annotate -x $2 $FILEIN -Ou | bcftools norm -m - -Ou | bcftools view -f PASS -Ou | bcftools filter -i $3 -Oz -o $FILEOUT --threads $4
    elif [ "$2" != "NA" ] && [ "$3" == "NA" ]
    then
        bcftools annotate -x $2 $FILEIN -Ou | bcftools norm -m - -Ou | bcftools view -f PASS -Oz -o $FILEOUT --threads $4
    else
        bcftools norm -m - $FILEIN -Ou | bcftools view -f PASS -Ou | bcftools filter -i $3 -Oz -o $FILEOUT --threads $4
    fi
